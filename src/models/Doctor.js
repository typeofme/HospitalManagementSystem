...
class Doctor {
  ...
  static async getWithSpecializations(doctorId) {
    return await db("doctors")
      ...
      .leftJoin(
        "treatments",
        "doctor_specializations.treatment_id",
        "treatments.id"
      )
  ...
  static async getTotalDoctorsFunction() {
    const result = await db.raw("SELECT get_total_doctors() as total");
    return result[0][0].total;
  }

  static async getDoctorsByDepartmentFunction(departmentId) {
    const result = await db.raw(
      "SELECT get_doctors_by_department(?) as total",
      [departmentId]
    );
    return result[0][0].total;
  }
