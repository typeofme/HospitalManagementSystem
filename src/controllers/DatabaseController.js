...
class DatabaseController {
  ...
  static async getCurrentPatientsCount(req, res) {
    try {
      const result = await db.raw('SELECT GetCurrentPatientsCount() as patient_count');
      res.json({ 
        success: true,
  ...
  static async calculatePatientAge(req, res) {
    try {
      ...
      const result = await db.raw('SELECT CalculatePatientAge(?, ?) as patient_age', [patient_id, date]);
  ...
  static async getAppointmentDetailsView(req, res) {
    try {
      ...
      const totalCount = await db('appointment_details_view').count('* as count');
      const totalPages = Math.ceil(totalCount[0].count / limit);
  ...
  static async testTriggers(req, res) {
    try {
      ...
      try {
        ...
        testResults.push({ test: 'Patient Update Trigger', status: 'FAILED', error: error.message });
      }
      ...
      try {
        ...
        testResults.push({ test: 'Past Date Trigger', status: 'FAILED - Should have been blocked' });
      } catch (error) {
        testResults.push({ test: 'Past Date Trigger', status: 'SUCCESS - Correctly blocked past date' });
      }
